(function ($, global) {
    ...
    function loadShake(){
            ...
            $.fn.jshaker = function(opts){
                options = $.extend(options, opts);
                element = $(this);
                
                var iFunc = function(){ animate(element); };
                setTimeout(iFunc, 50);
                setTimeout(function() {
                    stop(element);
            ...
            var animate = function(el){
                function random() {
                    return Math.random() * 6 < 3 ? (Math.random() * 6 * (-1)) : Math.random() * 6;
                }
                el.find('*').each(function(idx, item) {
                    var transform = 'translate('+ random() + 'px,' + random() + 'px)';
                    $(item).css({
                        '-webkit-transform': transform,
                        'transform': transform
                    });
                });
                var iFunc = function(){ animate(el); };
